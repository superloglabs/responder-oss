...
describe("browser analytics", () => {
  ...
  it("does not load analytics when the project token is absent", async () => {
    ...
    const { captureBrowserPageView, allowBrowserAnalytics } = await import(
      "./browser-analytics"
    );

    await allowBrowserAnalytics();
  ...
  it("waits for measurement consent before loading analytics", async () => {
    vi.stubEnv("VITE_POSTHOG_PROJECT_TOKEN", "phc_test");
    const { allowBrowserAnalytics, captureBrowserPageView } = await import(
      "./browser-analytics"
    );

    const pageView = captureBrowserPageView("https://responder.example/");
  ...
  it("does not load analytics to reset an identity without consent", async () => {
    vi.stubEnv("VITE_POSTHOG_PROJECT_TOKEN", "phc_test");
  ...
  it("uses configured analytics and starts masked session recording", async () => {
    ...
    const { allowBrowserAnalytics } = await import("./browser-analytics");
  ...
  it("treats analytics initialization failures as optional and retries", async () => {
    ...
    await expect(allowBrowserAnalytics()).resolves.toBeUndefined();
    await expect(allowBrowserAnalytics()).resolves.toBeUndefined();
  ...
  it("links authenticated browser activity to the user and organization", async () => {
    vi.stubEnv("VITE_POSTHOG_PROJECT_TOKEN", "phc_test");
    const { allowBrowserAnalytics, identifyBrowserUser } = await import(
      "./browser-analytics"
    );
    await allowBrowserAnalytics();
  ...
  it("clears the browser identity on sign out", async () => {
    vi.stubEnv("VITE_POSTHOG_PROJECT_TOKEN", "phc_test");
    const { allowBrowserAnalytics, resetBrowserAnalytics } = await import(
      "./browser-analytics"
    );
    await allowBrowserAnalytics();
