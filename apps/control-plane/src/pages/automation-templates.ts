...
export const automationTemplates: AutomationTemplate[] = [
  ...
  },
  {
    ...
    trigger: { ...weeklyAtNine, frequency: "hourly" },
  },
  {
    category: "scans",
...
export function applyAutomationTemplate(configuration: AutomationConfiguration, template: AutomationTemplate, options: AutomationOptions, timezone = browserTimeZone()): AutomationConfiguration {
  const { trigger } = template;
  const triggerAccountId = trigger.kind === "schedule" ? "" : options.accounts.find((account) => account.provider === trigger.kind)?.id ?? "";
...
export function automationTemplateMissingFields(template: AutomationTemplate): string {
  const { kind } = template.trigger;
  if (kind === "schedule") return "a repository";
  const resource = kind === "sentry" ? "a Sentry project" : kind === "discord" ? "a Discord channel" : "a Slack channel";
